...
object Main extends App {
    ...
    def deleteHead(cnt: Int, list: List[Node]): List[Node] = {
        if (cnt == k){
            return list
        } else {
            val current = list(0)
            val updatedList = list.updated(0, current.updateNext(list(current.next).next))
            deleteHead(cnt+1, updatedList)
        }
    }
    ...
    val finalList = deleteHead(0, loadList)
